...
p1 = ggplot(combined@meta.data, aes(x=celltype, fill=cond)) + 
...
p2 = ggplot(combined@meta.data, aes(x=celltype, fill=cond.sex.batch)) + 
  geom_bar(position = "fill", color = "black") +
  theme_classic(base_size = 10) +
  labs(x = '', y = 'Proportion of cell (%)') +
...
ggsave(p, file = "Figures/BarPlot_Seurat_Integrated_DraftAnnot_Cluster_Proportion.pdf",
       width = 15, height = 4)
...
## Basic wilcoxon-test
deg.list = list()
for (celltype in unique(combined$celltype)){
  print(celltype)
...
deg.list2 <- lapply(deg.list, set_gene_name) # Convert rowname to gene_name column
...
# 
# saveRDS(df_LR, "Data/DEG_Condition_LR_Latent_BindList_240219.RDS")


######################################################
### Visualization
# "CR"     "Oligo"  "Ext-2"  "Micro"  "Inh-1"  "Ext-6"  "Endo"   "Ext-1"  
# "Astro"  "Ext-4"  "Ext-3"  "Ext-DG" "OPC"    "Ext-5"  "Inh-2" 

## Basic Wilcoxon
for (celltypes in unique(combined$celltype)){
  
  res1 = df[df$celltype == celltypes,]
  ...
  p = ggplot(res1, aes(x = avg_log2FC, y = -log10(p_val_adj) )) + 
    ...
    labs(title = paste0('DEG in ', celltypes, ': Test vs. Control (Basic)')) + # Add a title
...
for (celltypes in unique(combined$celltype)){
  ...
  p1 = ggplot(res1, aes(x = avg_log2FC, y = -log10(p_val_adj) )) + 
    geom_point_rast(aes(fill = Change), 
                    shape = 21, alpha = 0.75,
                    na.rm = F, stroke = 0, size=2.5) + # Make dots bigger
    theme_classic(base_size = 15) + # change theme
  ...
  res2 = df_mast[df_mast$celltype == celltypes,]
  res2$Change = ifelse(res2$p_val_adj < 0.05 & abs(res2$avg_log2FC) > 0.25,
                       ifelse(res2$avg_log2FC > 0, "Up", "Down"), "None")
  x_range = max(abs(res2$avg_log2FC)) * 1.05
  res.gene = res2[res2$Change != "None",]$gene_name
  
  p2 = ggplot(res2, aes(x = avg_log2FC, y = -log10(p_val_adj) )) + 
    geom_point_rast(aes(fill = Change), 
                    shape = 21, alpha = 0.75,
                    na.rm = F, stroke = 0, size=2.5) + # Make dots bigger
    theme_classic(base_size = 15) + # change theme
    ...
    geom_vline(xintercept = -0.25, linetype='dashed', colour = "grey40", size = 0.7) + # Add 0 lines
    ggplot2::xlim(-x_range, x_range) + 
    geom_text_repel(d = subset(res2, res2$gene_name %in% res.gene),
                    aes(label = gene_name),
                    size = 3.5) +
    theme(
      panel.grid.major = element_line(colour = "grey80", size = 0.5),
  ...
  print(paste0(celltypes, " Basic Up: ", nrow(res1[res1$Change == "Up",]), 
               " Basic Down: ", nrow(res1[res1$Change == "Down",])))
  print(paste0(celltypes, " Mast Up: ", nrow(res2[res2$Change == "Up",]),
               " Mast Down: ", nrow(res2[res2$Change == "Down",], "\n")))
