...
source("~/Dropbox/CodeShareHub/EnrichmentPlot/run_enrichment_v8_GOBP.R")

df_mast = readRDS("Data/DEG_Condition_Mast_Latent_BindList_240519.RDS")
colnames(df_mast) 
...
for (cluster in unique(df_mast$celltype)) {
  tryCatch({
    ...
    res = run_all(deg_cluster_tmp,
                  ...
                  minGSSize = 100,
                  maxGSSize = 1000,
                  output_file = paste0('Tables/cluster_', cluster, '_enrichment_result_0519.xlsx'))
    ...
    res_pl = plot_all(res,
                      ...
                      lfc_thresh = 0.25,
                      padj_cutoff = 0.05,
                      output_file = paste0('Figures/cluster_', cluster, '_enrichment_result_0519.pdf'))
  }, error = function(e) {
